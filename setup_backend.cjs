...
`,
  ...
  // Models
  ...['User', 'Kitchen', 'MenuItem', 'Category', 'Order', 'OrderItem', 'Address', 'Coupon', 'Review', 'Notification', 'Driver', 'Payment', 'Wishlist'].reduce((acc, name) => {
    acc[`models/${name}.ts`] = `import { Schema, model, models } from 'mongoose';\n\nconst ${name}Schema = new Schema({}, { timestamps: true });\n\nexport const ${name} = models.${name} || model('${name}', ${name}Schema);\n`;
    return acc;
  }, {}),

  // Schemas
  ...['user', 'login', 'register', 'address', 'order', 'coupon', 'menu', 'kitchen'].reduce((acc, name) => {
    acc[`schemas/${name}.ts`] = `import { z } from 'zod';\n\nexport const ${name}Schema = z.object({});\n`;
    return acc;
  }, {}),
...
`,
  ...
  // Types
  ...['user', 'order', 'menu', 'kitchen', 'cart', 'address', 'payment', 'coupon', 'driver', 'review'].reduce((acc, name) => {
...
// Write API Routes
const apiRoutes = ['auth', 'menu', 'orders', 'checkout', 'kitchen', 'payment', 'review', 'offers', 'notifications'];
apiRoutes.forEach(route => {
  const dirPath = path.join(src, 'app', 'api', route);
  fs.mkdirSync(dirPath, { recursive: true });
  fs.writeFileSync(path.join(dirPath, 'route.ts'), `import { NextResponse } from 'next/server';\n\nexport async function GET() {\n  return NextResponse.json({ message: '${route} API placeholder' });\n}\n`);
});
