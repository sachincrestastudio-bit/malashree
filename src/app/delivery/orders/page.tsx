    ...
    <div className="max-w-lg mx-auto space-y-4 pb-6">
      ...
      {assignments.length === 0 ? (
        ...
        assignments.map((order: any) => (
          <div key={order._id} className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
            {/* Order Header */}
            ...
            `}>
              ...
              <div className="text-right">
                <span className="text-white font-bold">₹{order.grandTotal?.toFixed(2) || '0.00'}</span>
              </div>
            </div>
            ...
            <div className="p-4 space-y-4">
              <div className="flex gap-4">
                ...
                <div className="pb-4 flex-1">
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Pickup From</p>
                  <p className="text-white font-bold">{order.kitchenName}</p>
                  <p className="text-slate-400 text-sm">{order.kitchen?.location?.address || 'Kitchen Address'}</p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Deliver To</p>
                  <p className="text-white font-bold">{order.customer?.name || 'Customer'}</p>
                  <p className="text-slate-400 text-sm truncate pr-4">{order.deliveryAddress?.street}</p>
                </div>
              </div>
            </div>
