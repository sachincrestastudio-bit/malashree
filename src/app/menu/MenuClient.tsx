...
interface MenuClientProps {
  ...
  initialKitchenArea?: string;
}

export default function MenuClient({
  initialDishes,
  initialKitchenName = "Malashree Pure Veg",
  initialKitchenArea = "Pune",
}: MenuClientProps) {
  ...
  // Re-fetch when branchId or search changes
  useEffect(() => {
    let mounted = true;
    ...
    const timer = setTimeout(loadDishes, search.trim() ? 250 : 0);
    return () => {
      mounted = false;
      clearTimeout(timer);
    };
  ...
  }, [dbDishes, selectedCategory, search, vegOnly]);
  ...
  const groupedDishes = useMemo(() => {
    ...
    const groups: { categoryName: string; items: MenuItemData[] }[] = [];
  ...
  const cartTotal = cart.reduce((sum, i) => {
    const dish = dbDishes.find((d) => d.id === i.dishId);
            ...
            <div className="flex items-start sm:items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-[#52635c] mt-1 font-medium">
                  Authentic Royal Thalis, Chole Bhature, Biryani, Curries & Desserts
                </p>
                <p className="text-xs text-[#52635c] mt-0.5 flex items-center gap-1">
                  <MapPin className="size-3.5 text-[#d4af37]" />
                  <span>Auto-routed from closest kitchen: <b>{userLocation?.kitchenName || initialKitchenName}</b> ({userLocation?.label || initialKitchenArea})</span>
                </p>
                ...
                <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-[#52635c] font-medium">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {userLocation?.etaMin || 22} mins delivery
                  </span>
                  <span>•</span>
          ...
          <div className="mt-5 pt-4 border-t border-[#e6e2d8]">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search across all pure veg dishes (e.g. thali, chole bhature, biryani, paneer, naan)..."
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
            {categories.map((cat) => {
              const count =
                cat === "All"
                  ? dbDishes.length
                  : dbDishes.filter((d) => d.category.toLowerCase().trim() === cat.toLowerCase().trim()).length;
        ...
        <div className="lg:grid lg:grid-cols-12 lg:gap-8 lg:items-start">
          ...
          <aside className="hidden lg:block lg:col-span-3 lg:sticky lg:top-24 space-y-3">
            ...
            <div className="bg-white rounded-3xl p-5 border border-[#e6e2d8] shadow-2xs space-y-2.5">
              ...
              <p className="text-xs font-semibold text-[#0d261e]">
                {userLocation?.kitchenName || initialKitchenName}
              </p>
              <p className="text-[11px] text-[#52635c]">
                Automatically routed based on your location for fastest delivery.
              </p>
            </div>
          </aside>
