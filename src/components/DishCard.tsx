  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-ink/5">
        ...
        <button onClick={() => toggle(dish.id)} aria-label="Save"
          ...
          <Heart className={`size-3.5 transition ${isFav ? "fill-lime text-lime" : "text-cream group-hover/fav:text-ink"}`} />
        </button>
        {dish.tag && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5">
            <span className="h-px w-4 bg-lime" />
            <span className="text-[9px] tracking-[0.28em] uppercase text-lime font-semibold">{dish.tag}</span>
            <span className="h-px w-4 bg-lime" />
          </div>
        )}
        {/* Bottom overlay meta */}
        <div className="absolute bottom-0 left-0 right-0 p-4 text-cream">
          <div className="flex items-center gap-2 text-[9px] uppercase tracking-[0.22em] text-cream/70 font-mono">
            <span className={`size-2 border ${dish.veg ? "border-lime" : "border-red-400"} grid place-items-center`}>
              <span className={`size-1 rounded-full ${dish.veg ? "bg-lime" : "bg-red-400"}`} />
            </span>
            <span>{dish.category}</span>
            <span className="opacity-40">/</span>
            <span className="flex items-center gap-1"><Star className="size-2.5 fill-lime text-lime" />{dish.rating}</span>
          </div>
        ...
        <div className="mt-4 pt-3 border-t border-ink/10 flex items-end justify-between gap-2">
          <div>
            ...
            <div className="font-display text-2xl sm:text-3xl text-ink leading-none mt-0.5">
              <span className="text-lime">₹</span>{dish.price}
            </div>
          </div>
          {inCart ? (
            <div className="flex items-center gap-1 border border-ink text-ink h-9">
              <button onClick={() => setQty(dish.id, inCart.qty - 1)} className="size-9 grid place-items-center hover:bg-ink hover:text-cream transition">
